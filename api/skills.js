...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'POST') {
            const { name, category, status, level, notes } = req.body;
            if (!name || !category) {
            ...
            const { data, error } = await supabase
                .from('skills')
                .insert({ name, category, status, level, notes })
                .select()
                .single();
        ...
        if (req.method === 'PUT') {
            if (!id) return res.status(400).json({ error: 'Skill ID is required for update.' });

            const { name, category, status, level, notes } = req.body;
            const { data, error } = await supabase
                .from('skills')
                .update({ name, category, status, level, notes })
                .eq('id', id)
                .select()
                .single();
        ...
        if (req.method === 'DELETE') {
            if (!id) return res.status(400).json({ error: 'Skill ID is required for deletion.' });
